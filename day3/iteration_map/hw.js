...
const {recipes} = data;
// iteration to get only names and cuisines from recipes


for(let i=0;i<recipes.length;i++){
    const recipe = recipes[i];
    console.log(`Recipe Name: ${recipe.name}, Cuisine: ${recipe.cuisine}`);
}

// Using forEach to achieve the same result
recipes.forEach(recipe =>  {
    console.log(`Name: ${recipe.name}, Cuisine: ${recipe.cuisine}`);
});
...
for(let i = 0; i < recipes.length; i++) {
    const recipe = recipes[i];
    const {name, cuisine} = recipe; // Destructuring to get name and cuisine
    // Pushing an object with name and cuisine to newArr
    newArr.push({name: recipe.name, cuisine: recipe.cuisine});
}

console.log(newArr);

// Using map to achieve the same result in a more concise way
const mappedArr = recipes.map(recipe => {
    const {name, cuisine} = recipe; // Destructuring to get name and cuisine
   

    
    return {name, cuisine}; // Returning an object with name and cuisine
});
console.log(mappedArr);

// Filtering recipes with rating greater than 4.5 and returning only name and cuisine
// Using filter and map together
const filtered = recipes.filter(recipe => {
    const {rating} = recipe; // Destructuring to get name and cuisine
    if(rating > 4.5) return true; // Returning an object with name and cuisine
    else return false; // Returning an object with name and cuisine
});
const finalArr = filtered.map(recipe => {
    const {name, cuisine} = recipe; // Destructuring to get name and cuisine
    return {name, cuisine}; // Returning an object with name and cuisine
});
console.log(finalArr);

//use map and filter together to get names of recipes with rating greater than 4.5
const highRatedRecipes = recipes
    .filter(recipe => recipe.rating > 4.5) // Filter recipes with rating greater than 4.5
    .map(recipe => ({name: recipe.name, cuisine: recipe.cuisine})); // Map to get only the names
